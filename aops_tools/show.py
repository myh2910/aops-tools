...
def show_post_data(posts_data, topic_title, print_item, colors=COLORS[0]):
    ...
    Args:
        posts_data (list of dict): Data of the posts to display.
        topic_title (str): Topic title.
        print_item (function): Determines if the data of a post will be
        displayed.
        colors (tuple of str and None): Title and post property name colors.
    """
    ...
    for idx, data in enumerate(posts_data):
        ...
        if print_item(
            ...
            info = [
                ...
                (
                    "Posted by",
                    f"{data['username']} ({utils.PROFILE_URL % data['poster_id']})",
                ),
                ("Posted at", post_time),
            ]
            if data["thankers"]:
...
def show_view_posts_data(items_data, category_name, print_item, colors=COLORS[1]):
    ...
    Args:
        items_data (list of dict): Data of the items to display.
        category_name (str): Category name.
        print_item (function): Determines if the data of an item will be
        displayed.
        colors (tuple of str and None): Title and property name colors.
    """
    for idx, data in enumerate(items_data):
        ...
        if print_item(
            ...
            if canonical and (CONFIG["brave"] or data["item_type"] != "post_hidden"):
                utils.print_text(canonical)
...
def show_folder_data(items_data, print_item, colors=COLORS[0]):
    ...
    for idx, data in enumerate(items_data):
        if print_item(
            text := data["item_text"].strip(), subtitle := data["item_subtitle"].strip()
        ):
            utils.print_centered("Item", colors[0])
            info = [
...
def show_topic_data(data, stalk_users, find_posts, colors=COLORS[1]):
    ...
    Args:
        ...
        stalk_users (set of str): Users to stalk in this topic.
    ...
    info.extend([("Post count", data["num_posts"]), ("View count", data["num_views"])])
    for prop in info:
        utils.print_wrapped(prop, colors[1])
...
def show_category_data(data, find_text, colors=COLORS[1]):
    """Show AoPS category data on the terminal. An AoPS category consists of
    several types, where only the `view_posts` and `folder` types will be
    displayed.

    Args:
        data (dict): Category data.
        find_text (str): Text to find in this category.
        colors (tuple of str and None): Title and property name colors.
    """
    utils.print_centered("Category", colors[0])
    info = [("Link", data["category_url"]), ("Name", data["category_name"])]
    ...
    print_item = lambda str1, str2: not CONFIG["silent"] and (
        CONFIG["verbose"] or (find_text and (find_text in str1 or find_text in str2))
    )
